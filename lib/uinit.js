...
this.uinit = function () {
    ...
    function dependingOn(dep, features) {
        return C.select(function (feature) {
    ...
    appInstance = bane.createEventEmitter({
        ...
        nullableEnv: function (name, value) {
            if (this.env[name]) {
                var toRetry = dependingOn(name, this.features);
                reset(toRetry);
                tryFeatures(this, toRetry);
            } else {
                // Verify uniqueness if env variable has never been set before.
                // Overwriting env vars is allowed, but writing an env var over
                // an existing feature/data is not.
