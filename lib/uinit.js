...
this.uinit = function () {
     ...
     */
    function tryFeatureInContext(app, features, feature, context) {
        var load = C.partial(loadFeature, app, features, feature);
        if (feature.elements) {
            return when.all(C.map(load, dome.byClass(feature.elements, context)));
        } else {
            return load();
        }
    }
    ...
    function logPending(app, features) {
        if (!app.listeners.pending) { return; }
    ...
    function tryFeatures(app, features, context) {
        ...
        function tryNext() {
            ...
            if (!feature) { return logPending(app, featureArr); }

            tryFeatureInContext(app, features, feature, context).then(function () {
                if (!feature.loaded) {
    ...
    function addFeature(app, feature) {
        ensureUnique(app.features, feature.name);
    ...
    appInstance = bane.createEventEmitter({
        ...
        feature: function (name, fn, opt) {
            if (typeof fn !== "function") {
                throw new Error("Cannot add feature " + name +
                                ", action is not a function (" + typeof fn +
                                ")");
            }
